...
export default function sortActions(actions: IAction[]) {
  ...
  actions.sort((left: IAction, right: IAction) => {
    if (orderedActionTypes.indexOf(left.actionType) < orderedActionTypes.indexOf(right.actionType)) {
      return -1;
    }
    if (orderedActionTypes.indexOf(left.actionType) > orderedActionTypes.indexOf(right.actionType)) {
      return 1;
  ...
  for (let i = 0; i < actions.length; i++) {
    ...
    for (let j = 0; j < actions.length; j++) {
      ...
      if (rightAction.depends.length === 0 || leftAction.actionType !== rightAction.actionType) {
        continue;
      }
